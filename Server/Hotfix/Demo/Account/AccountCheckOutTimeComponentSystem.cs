namespace ET {
    ...
    public static class AccountCheckOutTimeComponentAwakeSystem : AwakeSystem<AccountSessionCheckOutTimeComponent, long> {
        public override void Awake(AccountCheckOutTimeComponent self, long accountId) {
            self.AccountId = accountId;
            //ref 关键字，传入前必须初始化
            TimerComponent.Instance.Remove(ref self.Timer);
            self.Timer = TimerComponent.Instance.NewOnceTimer(TimeHelper.ServerNow() + 60000m TimerType.AccountSessionCheckOutTime, self);
        }
    }
    public static class AccountCheckOutTimeComponentDestroySystem : DestroySystem<AccountSessionCheckOutTimeComponent> {
