...
namespace ET {
    ...
    public struct InstanceIdStruct {
        ...
        public InstanceIdStruct(long id) { // 根据一个 actorId: 来变换得到这个参数所携带的信息
            ulong result = (ulong) id;
            this.Value = (uint)(result & IdGenerater.Mask18bit); // 低 18 位
            result >>= 18;
            this.Process = (int)(result & IdGenerater.Mask18bit); // 从后往前数，再 18 位
            result >>= 18;
            this.Time = (uint)result; // 最高位 28 位：所携带的信息
        }
