...
namespace ET {
    ...
    public sealed class EventSystem: IDisposable {
        ...
        private List<Type> GetBaseAttributes() {
            ...
            foreach (var kv in this.allTypes) { // 全类型：所有的类型
                Type type = kv.Value;
                if (type.IsAbstract) {
                    continue;
                }
        ...
        public void Add(Dictionary<string, Type> addTypes) {
            ...
            this.types.Clear(); // 先清空，拿全类型，所有的【标签标注过的】类型：问题变成是，我还需要再去检查这个标签系的扫描过程吗？先排除其它部分可能会存在的问题
            List<Type> baseAttributeTypes = GetBaseAttributes(); // 拿到的是，继承自 BaseAttribute 的所有子标签系类型
